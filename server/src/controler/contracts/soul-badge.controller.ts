...
const initialize = AsyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await soulBadgeService.initialize(adminKey);
  return res.status(200).json(new ApiResponse(200, result, 'Soul Badge initialized'));
});
...
const mint = AsyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await soulBadgeService.mint(reaperAddress, missionId, rank);
...
const revoke = AsyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await soulBadgeService.revoke(adminKey, reaperAddress, missionId);
...
const getBadges = AsyncHandler(async (req: Request, res: Response) => {
  ...
  const badges = await soulBadgeService.getBadges(reaperAddress);
...
const verifyBadge = AsyncHandler(async (req: Request, res: Response) => {
  ...
  const verified = await soulBadgeService.verifyBadge(reaperAddress, missionId);
...
const badgeCount = AsyncHandler(async (req: Request, res: Response) => {
  const { reaperAddress } = req.params;
  if (!reaperAddress) throw new ApiError(400, 'Reaper address is required');
  const count = await soulBadgeService.badgeCount(reaperAddress);
...
const totalBadges = AsyncHandler(async (req: Request, res: Response) => {
  const total = await soulBadgeService.totalBadges();
...
const getTopReapers = AsyncHandler(async (req: Request, res: Response) => {
  ...
  const topReapers = await soulBadgeService.getTopReapers(limit);
  return res.status(200).json(new ApiResponse(200, topReapers, 'Top reapers retrieved'));
});
