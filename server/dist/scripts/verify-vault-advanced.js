...
async function runSimulation() {
    ...
    try {
        ...
        try {
            await seireiteiVault.putWithIndex('Users', userId, userProfile, 'Email', userProfile.email);
        ...
        try {
            await seireiteiVault.putWithIndex('Missions', missionId, missionData, 'Assignee', userId);
        ...
        try {
            ...
            let currentMission = missionData; // Fallback
            try {
                const fetched = await seireiteiVault.get('Missions', missionId);
                if (fetched)
                    currentMission = fetched;
            }
            catch (_) { }
            // 3b. Add Proof
            ...
            // 3c. Update on-chain
            await seireiteiVault.put('Missions', missionId, currentMission);
        ...
        try {
            const retrievedId = await seireiteiVault.getByIndex('Users', 'Email', userProfile.email);
            if (retrievedId && retrievedId !== userId)
                throw new Error("Index Lookup Returned Wrong ID!");
...
//# sourceMappingURL=verify-vault-advanced.js.map