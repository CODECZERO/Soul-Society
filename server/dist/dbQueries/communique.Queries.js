...
const saveCommunique = async (data) => {
    try {
        const id = nanoid();
...
const getCommuniquesByDivision = async (ngoId) => {
    try {
        ...
        return all.filter(c => c.NgoRef === ngoId).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    }
    catch (error) {
...
//# sourceMappingURL=communique.Queries.js.map