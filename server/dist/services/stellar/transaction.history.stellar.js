import { Horizon } from '@stellar/stellar-sdk';
import dotenv from 'dotenv';
dotenv.config();
const HORIZON_URL = process.env.BLOCKCHAIN_NETWORK || 'https://horizon-testnet.stellar.org';
const server = new Horizon.Server(HORIZON_URL);
/**
 * Fetch transaction history for a Stellar public key.
 * Returns the most recent transactions (up to `limit`).
 */
export async function getTransactionHistory(publicKey, limit = 20, order = 'desc') {
    try {
        const txPage = await server
            .transactions()
            .forAccount(publicKey)
            .limit(limit)
            .order(order)
            .call();
        return txPage.records.map((tx) => ({
            id: tx.id,
            hash: tx.hash,
            ledger: tx.ledger_attr,
            created_at: tx.created_at,
            source_account: tx.source_account,
            fee_charged: tx.fee_charged,
            operation_count: tx.operation_count,
            memo_type: tx.memo_type,
            memo: tx.memo || undefined,
            successful: tx.successful,
        }));
    }
    catch (error) {
        if (error?.response?.status === 404) {
            return []; // Account not found or has no transactions
        }
        console.error('Error fetching transaction history:', error);
        throw error;
    }
}
/**
 * Fetch operation details for a specific transaction.
 */
export async function getTransactionOperations(txHash) {
    try {
        const ops = await server
            .operations()
            .forTransaction(txHash)
            .call();
        return ops.records.map((op) => ({
            id: op.id,
            type: op.type,
            source_account: op.source_account,
            created_at: op.created_at,
            ...op,
        }));
    }
    catch (error) {
        console.error('Error fetching transaction operations:', error);
        throw error;
    }
}
//# sourceMappingURL=transaction.history.stellar.js.map