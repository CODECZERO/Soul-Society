...
import { getTransactionHistory, getTransactionOperations, } from '../services/stellar/transaction.history.stellar.js';
// Get wallet balance
...
const getAccountTransactions = AsyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, transactions, `Found ${transactions.length} transactions`));
});
// Get operations for a specific transaction
const getTransactionOps = AsyncHandler(async (req, res) => {
    const { txHash } = req.params;
    if (!txHash)
        throw new ApiError(400, 'Transaction hash is required');
    const operations = await getTransactionOperations(txHash);
    return res.status(200).json(new ApiResponse(200, operations, `Found ${operations.length} operations`));
});
export { getWalletBalance, sendPayment, verifyTransaction, createStellarAccount, deleteStellarAccount, saveToSmartContract, getLatestContractData, getAccountTransactions, getTransactionOps, };
