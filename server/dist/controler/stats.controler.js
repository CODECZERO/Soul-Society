...
const getLeaderboard = AsyncHandler(async (req, res) => {
    try {
        ...
        const leaderboard = ngos.map((ngo) => {
            ...
            const missions = allPosts.filter((p) => p.NgoRef === ngo.id || p.NgoRef === ngo._id);
            const completedCount = missions.filter((m) => m.Status === 'Completed').length;
            const totalXLM = missions.reduce((sum, m) => sum + (m.CollectedAmount || 0), 0);
...
const getDonorStats = AsyncHandler(async (req, res) => {
    ...
    try {
        const donations = await getAllDonation();
        const donorDonations = donations.filter((d) => d.currentTxn && walletAddr && d.currentTxn.includes(walletAddr.substring(0, 5)));
        const totalXLM = donorDonations.reduce((sum, d) => sum + (d.Amount || 0), 0);
        const missionCount = new Set(donorDonations.map((d) => d.postIDs)).size;
        let badge = 'Academy Student';
        if (totalXLM > 500)
            badge = 'Captain';
        else if (totalXLM > 100)
            badge = 'Lieutenant';
        else if (totalXLM > 20)
            badge = 'Assistant Reaper';
        const donorInfo = {
            walletAddr,
 ...
 */
const getContributorLeaderboard = AsyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (ngoId) {
            const ngoPostIds = allPosts
                .filter((p) => p.ngo === ngoId || p.NgoRef === ngoId)
                .map((p) => p.id || p._id);
            filteredDonations = allDonations.filter((d) => ngoPostIds.includes(d.postIDs));
        }
        const donorStats = {};
        filteredDonations.forEach((d) => {
            ...
            if (d.postIDs)
                donorStats[donor].missionCount.add(d.postIDs);
        });
...
const getNGOStats = AsyncHandler(async (req, res) => {
    ...
    try {
        ...
        const allPosts = await getPosts();
        const XLM_TO_INR_RATE = await getXLMtoINRRate();
        const ngoPosts = allPosts.filter((p) => p.ngo === ngoId || p.NgoRef === ngoId);
        const ngoPostIds = ngoPosts.map((p) => p.id || p._id);
        const ngoDonations = allDonations.filter((d) => ngoPostIds.includes(d.postIDs));
        const ngoExpenses = allExpenses.filter((e) => ngoPostIds.includes(e.postIDs));
        const totalRaisedXLM = ngoDonations.reduce((sum, d) => sum + (d.Amount || 0), 0);
        const totalSpentXLM = ngoExpenses.reduce((sum, e) => sum + (e.Amount || 0), 0);
        const totalRaisedINR = Math.round(totalRaisedXLM * XLM_TO_INR_RATE);
        ...
        const stats = {
            ...
            activeTasks: ngoPosts.filter((p) => p.Status !== 'Completed').length,
            chartData
        };
