...
import { getDonation, getAllDonation, getDonationRelatedToPost, createDonation, } from '../dbQueries/donation.Queries.js';
...
// ─── Create Escrow XDR ─────────────────────────────────────────────
const getEscrowXdr = AsyncHandler(async (req, res) => {
    ...
    const xdr = await escrowService.buildCreateEscrowTx(donorPublicKey, ngoPublicKey, Number(totalAmount), Number(lockedAmount), taskId, Number(deadline));
...
const createNewDonation = AsyncHandler(async (req, res) => {
    ...
    const result = await createDonation({
        ...
        postID: postId,
        Amount: amount,
    }); // Cast as any or match the interface exactly if available
