...
fn setup() -> (Env, SeireiteiVaultClient<'static>, Address) {
    let env = Env::default();
    env.mock_all_auths();
    let contract_id = env.register(SeireiteiVault, ());
    let client = SeireiteiVaultClient::new(&env, &contract_id);
...
fn test_put_and_get() {
    ...
    let collection = SorobanString::from_str(&env, "Users");
    let id = SorobanString::from_str(&env, "U001");
...
fn test_has_existing_and_missing() {
    ...
    let collection = SorobanString::from_str(&env, "Posts");
    ...
    // Non-existent
    let missing = SorobanString::from_str(&env, "P999");
...
fn test_delete_entry() {
    ...
    let collection = SorobanString::from_str(&env, "Users");
    let id = SorobanString::from_str(&env, "U_DEL");
...
fn test_migrate_hot_to_cold() {
    ...
    let collection = SorobanString::from_str(&env, "Missions");
    let id = SorobanString::from_str(&env, "M_MIGRATE");
...
fn test_batch_put() {
    ...
    let collections = vec![
        &env,
        SorobanString::from_str(&env, "Users"),
        SorobanString::from_str(&env, "Users"),
        SorobanString::from_str(&env, "Posts"),
    ];
    let ids = vec![
        &env,
...
fn test_delta_update() {
    ...
    let collection = SorobanString::from_str(&env, "Users");
    let id = SorobanString::from_str(&env, "DELTA_USR");
...
fn test_metadata_stored() {
    ...
    let collection = SorobanString::from_str(&env, "Donations");
