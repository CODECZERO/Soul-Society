...
// ═══════════════════════════════════════════════════════════════════
//  DIVISION TREASURY v2 — MULTI-SIG, BUDGET, AUDIT TRAIL
// ═══════════════════════════════════════════════════════════════════
...
impl DivisionTreasury {
    ...
    pub fn deposit(env: Env, divider: Address, amount: i128, purpose: String) {
        ...
        Self::log_tx(&env, &divider, TxType::Deposit, amount, &purpose);
        env.events().publish((symbol_short!("treasury"), symbol_short!("deposit")), (divider, amount));
    }

    // ── WITHDRAWAL (with multi-sig for large amounts) ────────────

    pub fn withdraw(env: Env, divider: Address, amount: i128, purpose: String) {
        divider.require_auth();
        if amount <= 0 { panic!("Amount must be positive"); }
        ...
        if amount > threshold {
            ...
            let request = WithdrawalRequest {
                amount, purpose, requester: divider.clone(),
            ...
            env.events().publish((symbol_short!("treasury"), symbol_short!("req")), (divider, amount));
        } else {
            // Direct withdrawal
            let mut balance: i128 = env.storage().persistent().get(&DataKey::Balance(divider.clone())).unwrap_or(0);
            ...
            balance -= amount;
            env.storage().persistent().set(&DataKey::Balance(divider.clone()), &balance);
            Self::log_tx(&env, &divider, TxType::Withdrawal, amount, &String::from_str(&env, "direct"));
            env.events().publish((symbol_short!("treasury"), symbol_short!("withdraw")), (divider, amount));
        }
    }
    ...
    pub fn set_budget(env: Env, admin: Address, division: Address, max_amount: i128) {
        admin.require_auth();
        Self::require_admin(&env, &admin);
        env.storage().persistent().set(&DataKey::Budget(division), &max_amount);
    }

    pub fn get_budget(env: Env, division: Address) -> i128 {
        env.storage().persistent().get(&DataKey::Budget(division)).unwrap_or(0)
    }
    ...
    pub fn get_history(env: Env, divider: Address) -> Vec<TreasuryTx> {
        env.storage().persistent().get(&DataKey::TxHistory(divider)).unwrap_or(vec![&env])
    }
    ...
    fn log_tx(env: &Env, divider: &Address, tx_type: TxType, amount: i128, purpose: &String) {
        let mut history: Vec<TreasuryTx> = env.storage().persistent()
            .get(&DataKey::TxHistory(divider.clone())).unwrap_or(vec![env]);
        history.push_back(TreasuryTx {
            tx_type, amount, purpose: purpose.clone(),
            timestamp: env.ledger().timestamp(), actor: divider.clone(),
        });
        env.storage().persistent().set(&DataKey::TxHistory(divider.clone()), &history);
    }
